    ...
    public class MixedRealityTeleport : Singleton<MixedRealityTeleport>, IControllerInputHandler
    {
        ...
        private void HandleGamepad()
        {
            ...
            if (EnableStrafe)
            {
				...
				//DoStrafe(new Vector3(leftX, 0.0f, leftY) * StrafeAmount);
            }
        ...
        void IControllerInputHandler.OnInputPositionChanged(InputPositionEventData eventData)
        {
            if (eventData.PressType == InteractionSourcePressInfo.Thumbstick)
            {
                ...
                {
					thumbstickPos = new Vector3(eventData.Position.x, 0.0f, eventData.Position.y);
                }
		...
		public void PlayerMove() {
			Vector3 moveDirection = thumbstickPos.normalized;
			moveDirection.y = 0.0f;
			float moveAmount = thumbstickPos.magnitude;
			Transform relativeTo = CameraCache.Main.transform;
			Vector3 rot = relativeTo.rotation.eulerAngles;
			rot.y = 0.0f;
			relativeTo.Rotate(rot);
			transform.Translate(moveDirection * moveAmount * StrafeAmount * Time.deltaTime, relativeTo);
		}
